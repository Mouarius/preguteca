...
def get_or_create_category(category_name, category_fullname):
    ...
    if not category_qs:
        category_qs = Category.objects.create(
            name=category_name, full_name=category_fullname
        )
    return category_qs
...
class Command(BaseCommand):
    ...
    video_data_json_path = os.path.join(
        settings.BASE_DIR, "question_library/data/video_data__parsed.json"
    )
        ...
        with open(self.video_data_json_path) as file:
            ...
            for video in video_entries:
                category = get_or_create_category(
                    video["category_name"], video["category_fullname"]
                )
                if not category:
                    raise CommandError(
