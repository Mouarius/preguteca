...
function getMatchedCategories(
  ...
  return categoriesList.filter((cat) =>
    cat.fullName.toLocaleLowerCase().startsWith(searchText)
  );
}
...
function handleSuggestionHover(event: MouseEvent) {
  ...
  activeChoice.key =
    parseInt((event.target as HTMLElement).dataset.key ?? "0", 10) ?? 0;
  ...
  switch (event.key) {
    case "ArrowDown":
      if (activeChoice.key <= 8) {
        activeChoice.key += 1;
        activeChoice.category = getSelectedCategory(
          matchedCategoryElements.value
        );
      }
      break;
    case "ArrowUp":
    ...
    >
      <ul>
        <li
          ...
          :class="{ active: index === activeChoice.key }"
          :key="category.id"
          :data-key="index"
          ref="matchedCategoryElements"
          :data-category-id="category.id"
          class="suggestion__element"
